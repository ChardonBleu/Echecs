...
class PlayerController:
    ...
    def sort_players_by_score_and_ranking(self, player_manager):
        ...
        Args:
            player_manager {instance de PlayerManager} -- Contient la liste des 8 joueurs du tournoi courant
        """
        sorted_player_list = sorted(player_manager.couple_items(), key=lambda couple: couple[1].tournament_score, reverse = True)
        # sorted_player_list = sorted(sorted_player_list, key=lambda couple: couple[1].ranking,  reverse=True)
        player_manager.decouple_items(sorted_player_list)
